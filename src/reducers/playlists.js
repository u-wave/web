...
export default function reduce(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case UPDATE_MEDIA_START:
      return updatePlaylistItems(state, payload.playlistID, items => items.map(media => (
    ...
    case MOVE_MEDIA_START: {
      ...
      return updatePlaylistItems(state, payload.playlistID, items => items.map(media => media && ({
        ...media,
        loading: isMovingMedia[media._id] || media.loading,
      })));
    }
    case MOVE_MEDIA_COMPLETE:
      return updatePlaylistItems(state, payload.playlistID, items => processMove(items, payload.medias, payload.location));
